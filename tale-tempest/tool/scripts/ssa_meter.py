...
class FrequencyAnalyser:
    ...
    async def get_system_information(self):
        ...
        logger.ILine(f"get_network_information end {idn}")
        return idn
    ...
    async def set_system_parameters(self):
        logger.ILine(f"set_general_parameters begin")
    ...
    async def set_scan_parameter(self, freq):
        logger.ILine(f"set_frequency_parameter begin")
        ...
        logger.ILine(f"set_frequency_parameter end")
...
async def meter_once(frequencies=[]):
    ...
    try:
        ...
        if rt:
            ...
            await fa.get_network_information()

            await fa.get_system_information()
...
if __name__ == "__main__":
    logger.start('error')
    try:
        frequencies = sys.argv[1:] if len(sys.argv) > 1 else DEFAULT_FREQUENCIES
        asyncio.run(meter_once(frequencies))
    except KeyboardInterrupt as e:
        logger.ELine(traceback.format_exc())
        logger.Eine(e)